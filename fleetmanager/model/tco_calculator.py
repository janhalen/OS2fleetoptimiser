...
class TCOCalculator:
    ...
    def __init__(self, **kwargs):
        ...
        self.prisstigning_hvo = 1  # no valid reference for this variable
        self.pris_el = 2.13
        ...
        self.diesel_udledning = 2.98
        self.hvo_udledning = 0.894
        self.benzin_udledning = 2.52
        self.el_udledning = 0.09

        self.__dict__.update(kwargs)

        # Fra CO2e udledninger Fremskrivningsarket
        self.co2e_udledninger_diesel = np.full(30, self.diesel_udledning)
        self.co2e_udledninger_benzin = np.full(30, self.benzin_udledning)
        self.co2e_udledninger_hvo = np.full(30, self.hvo_udledning)
        if self.el_udledning == 0.09:
            ...
            self.co2e_udledninger_el = np.full(30, self.el_udledning)
        ...
        self.fremskrivning = {
            ...
            "hvo": self.drivmiddel_udvikling(self.pris_hvo, self.prisstigning_hvo),
        }
    ...
    def driftsomkostninger(self):
        ...
        """
        # todo restrict saving of wltp if the wrong category is selected
        # this will prompt error here, if for instance diesel is selected but the wltp is entered in wltp_el
        if self.drivmiddel not in ["benzin", "diesel", "el", "hvo"]:
            return [0]
        return [
            self.nutidsvaerdi_drift(
                self.aarlig_driftsomkostning(
    ...
    def ekstern_miljoevirkning(self, sum_it=False):
        udledninger = []
        aarligt_forbrug_benzin_diesel_hvo = (
            0
            if self.braendstofforbrug == 0 or pd.isna(self.braendstofforbrug)
            else self.koerselsforbrug / self.braendstofforbrug
        )
        if not pd.isna(self.elforbrug):
            ...
            el_aarligt_stroemforbrug = 0
            el_hybrid_aarligt_forbrug = 0
        if self.bil_type == "benzin":
            udledninger = [
                aarligt_forbrug_benzin_diesel_hvo * self.antal * k / 1000
        ...
        elif self.drivmiddel == "hvo":
            udledninger = [
                aarligt_forbrug_benzin_diesel_hvo * self.antal * k / 1000
                for k in self.co2e_udledninger_hvo
            ]
    ...
    def ekstern_miljoevirkning_summed(self) -> tuple[float, float]:
        udledninger, ekstern_virkninger = self.ekstern_miljoevirkning()
        return sum(udledninger), sum(ekstern_virkninger)
